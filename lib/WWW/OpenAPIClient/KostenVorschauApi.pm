...
#
package WWW::OpenAPIClient::KostenVorschauApi;
...
# @param int $month  (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'kosten_vorschau_api' } = {
        summary => '',
        params => $params,
        returns => 'KostenVorschau',
        };
}
# @return KostenVorschau
#
sub kosten_vorschau_api {
    my ($self, %args) = @_;

    # verify the required parameter 'year' is set
    unless (exists $args{'year'}) {
      croak("Missing the required parameter 'year' when calling kosten_vorschau_api");
    }

    # verify the required parameter 'month' is set
    unless (exists $args{'month'}) {
      croak("Missing the required parameter 'month' when calling kosten_vorschau_api");
    }

    # parse inputs
    my $_resource_path = '/api/v1/bookkeeping/kosten-vorschau';
    ...
    my $_response_object = $self->{api_client}->deserialize('KostenVorschau', $response);
