...
#
package WWW::OpenAPIClient::KstApi;
...
# @param string $gewinn  (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'kst_api' } = {
        summary => '',
        params => $params,
        returns => 'KstErgebnis',
        };
...
sub kst_api {
    ...
    unless (exists $args{'year'}) {
      croak("Missing the required parameter 'year' when calling kst_api");
    }

    # parse inputs
    my $_resource_path = '/api/v1/bookkeeping/kst';
    ...
    # query params
    if ( exists $args{'gewinn'}) {
        $query_params->{'gewinn'} = $self->{api_client}->to_query_value($args{'gewinn'});
    }
    ...
    my $_response_object = $self->{api_client}->deserialize('KstErgebnis', $response);
