...
#
package WWW::OpenAPIClient::AnlageGApi;
...
# @param int $year  (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'anlage_g_api' } = {
        summary => '',
        params => $params,
        returns => 'AnlageGErgebnis',
        };
}
...
sub anlage_g_api {
    ...
    unless (exists $args{'year'}) {
      croak("Missing the required parameter 'year' when calling anlage_g_api");
    }

    # parse inputs
    my $_resource_path = '/api/v1/bookkeeping/anlage-g';
    ...
    my $_response_object = $self->{api_client}->deserialize('AnlageGErgebnis', $response);
