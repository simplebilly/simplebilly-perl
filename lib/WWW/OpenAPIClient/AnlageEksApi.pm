...
#
package WWW::OpenAPIClient::AnlageEksApi;
...
# @param int $year  (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'eks_api' } = {
        summary => '',
        params => $params,
        returns => 'EksErgebnis',
        };
...
sub eks_api {
    ...
    unless (exists $args{'year'}) {
      croak("Missing the required parameter 'year' when calling eks_api");
    }

    # parse inputs
    my $_resource_path = '/api/v1/bookkeeping/eks';
    ...
    my $_response_object = $self->{api_client}->deserialize('EksErgebnis', $response);
